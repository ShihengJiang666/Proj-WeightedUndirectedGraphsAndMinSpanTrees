...
public class DList extends List {
  ...
  public void insertBack(Object item) {
    ...
    if (item instanceof DList){
      ((DList)item).myNode = new_Node;
    }
    if (item instanceof Edge){
      if (((Edge)item).myNode1==null) {
        ((Edge)item).myNode1 = new_Node;
      } else {
        ((Edge)item).myNode2 = new_Node;
      }
    }
    size++;
  }
  ...
  public void remove(Object sth){
    DListNode cur = (DListNode)front();
    while (cur.item != null){
      if (cur.item == sth){
        try{
          cur.remove();
